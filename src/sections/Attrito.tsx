  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      <text x={PAD.left + (dropX - PAD.left) / 2} y={PAD.top + iH - 6}
        textAnchor="middle" fill="rgba(46,125,50,0.5)" fontSize="9">statico</text>
      <rect x={dropX} y={PAD.top} width={PAD.left + iW - dropX} height={iH}
        fill="rgba(213,84,0,0.04)" />
      <text x={dropX + (PAD.left + iW - dropX) / 2} y={PAD.top + iH - 6}
        textAnchor="middle" fill="rgba(213,84,0,0.5)" fontSize="9">cinetico</text>
      <path d={staticPts.join(' ')} fill="none" stroke="#2e7d32" strokeWidth="2.5" />
      <path d={dropLine} fill="none" stroke="rgba(0,0,0,0.2)" strokeWidth="1.5" strokeDasharray="3 2" />
      <path d={kineticPts} fill="none" stroke="#e05600" strokeWidth="2.5" />
      ...
      <text x={14} y={PAD.top + iH / 2} textAnchor="middle" fill="rgba(0,0,0,0.45)" fontSize="10"
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="sim-svg">
      ...
      <text x={bx + blockW / 2} y={groundY - blockH / 2 + 5}
        textAnchor="middle" fill={moving ? '#e05600' : '#2563eb'} fontSize="12" fontWeight="700">{m}kg</text>
      {Fapp > 0 && (
        <>
          <line x1={bx + blockW} y1={groundY - blockH / 2}
            x2={bx + blockW + fScale(Fapp)} y2={groundY - blockH / 2}
            stroke="#b87800" strokeWidth="3" markerEnd="url(#arrA)" />
          <text x={bx + blockW + fScale(Fapp) / 2} y={groundY - blockH / 2 - 8}
            textAnchor="middle" fill="#b87800" fontSize="10">F={Fapp.toFixed(1)}N</text>
        </>
      )}
      {Fattrito > 0 && (
        <>
          <line x1={bx} y1={groundY - blockH / 2}
            ...
            stroke={moving ? '#e05600' : '#2e7d32'} strokeWidth="3" markerEnd={moving ? 'url(#arrFk)' : 'url(#arrFs)'} />
          <text x={bx - fScale(Fattrito) / 2} y={groundY - blockH / 2 - 8}
            textAnchor="middle" fill={moving ? '#e05600' : '#2e7d32'} fontSize="10">
...
const quizQ = [
  ...
  },
  {
    ...
    correct: 2,
    exp: "μ_s > μ_d: serve più forza per avviare il moto che per mantenerlo."
  },
  {
  ...
  },
  {
    ...
    correct: 1,
    exp: "Nel modello di attrito di Coulomb, μ_c è indipendente dalla velocità."
  },
  {
    q: "F applicata = 18 N, F_s_max = 25 N. Il blocco non si muove. La forza d'attrito statico vale:",
    opts: ["25 N", "0 N", "18 N", "7 N"],
    correct: 2,
    exp: "L'attrito statico è reattivo: assume il valore minimo necessario a tenere il blocco fermo, cioè 18 N."
  },
  {
    ...
    <>
      <div className="card">
        ...
        <div className="info-box warn">
          Serve più forza per <em>avviare</em> il moto che per <em>mantenerlo</em>. Per questo è più facile spingere un armadio già in movimento che farlo partire.
        </div>
