...
function solve(mode: Mode, m: number, F: number, mu: number, theta: number, _v0: number) {
  if (mode === 'orizzontale') {
    const N = m * G; const Fattr = mu * N; const Fnet = F - Fattr; const a = Fnet / m
    return { a, N, Fattr, Fpar: 0 }
  } else {
    const rad = (theta * Math.PI) / 180
    const N = m * G * Math.cos(rad); const Fpar = m * G * Math.sin(rad)
    const Fattr = mu * N; const a = G * (Math.sin(rad) - mu * Math.cos(rad))
    return { a, N, Fattr, Fpar }
  }
}

function stateAt(a: number, v0: number, t: number) {
  if (a < 0 && v0 > 0) { const tStop = v0/(-a); if (t >= tStop) return { v: 0, s: v0*tStop+0.5*a*tStop*tStop, stopped: true } }
  if (a > 0 && v0 < 0) { const tStop = (-v0)/a; if (t >= tStop) return { v: 0, s: v0*tStop+0.5*a*tStop*tStop, stopped: true } }
...
function SceneOriz({ m, F, mu, t, a, v0 }: { m: number; F: number; mu: number; t: number; a: number; v0: number }) {
  ...
  const groundY = 120, bW = 50, bH = 34, startX = 50
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="sim-svg">
      ...
      {F > 0 && <>
        <line x1={bx+bW} y1={groundY-bH/2} x2={bx+bW+fA} y2={groundY-bH/2} stroke="#e05600" strokeWidth="3" markerEnd="url(#aA)" />
...
const quizQ = [
  { q: "Un blocco di 6 kg su piano orizzontale, μ=0.2, F=30 N. L'accelerazione vale:", opts: ["5 m/s²","3.04 m/s²","2.96 m/s²","1.96 m/s²"], correct: 1, exp: "Fₐ = 11.76 N. Fnet = 18.24 N. a = 3.04 m/s²." },
  { q: "Blocco su piano a 45° senza attrito, v₀ = 8 m/s verso l'alto. Quanti secondi per fermarsi?", opts: ["0.82 s","1.15 s","1.63 s","0.58 s"], correct: 1, exp: "a = −g·sin45° = −6.93 m/s². t = 8/6.93 = 1.15 s." },
  { q: "Quale informazione NON serve per calcolare l'accelerazione su piano inclinato senza attrito?", opts: ["L'angolo θ","La massa del blocco","g","Tutte e tre servono"], correct: 1, exp: "a = g·sinθ: la massa si cancella." },
  { q: "Un blocco di 4 kg su piano a 30° con μ=0.3 (g=9.8). La forza normale vale:", opts: ["39.2 N","19.6 N","33.95 N","22.68 N"], correct: 2, exp: "N = 4×9.8×cos30° = 33.95 N." },
  { q: "Un blocco di 10 kg parte da fermo con a = 2 m/s². Spazio in 5 s:", opts: ["10 m","25 m","50 m","100 m"], correct: 1, exp: "s = ½×2×25 = 25 m." },
  { q: "Su piano a 37° con μ=0.75, il blocco scivola spontaneamente? (sin37°=0.6, cos37°=0.8)", opts: ["Sì, perché θ>0°","No, perché tan37°=0.75=μ, quindi a=0","Sì, perché μ<1","Non si può determinare"], correct: 1, exp: "a = g(0.6−0.75×0.8) = 0. Il blocco è in equilibrio!" },
  { q: "Blocco in discesa su piano a 40°, μ=0.2, v₀=5 m/s. Velocità dopo 3 s circa:", opts: ["5 m/s","16.9 m/s","14.7 m/s","11 m/s"], correct: 1, exp: "a ≈ 4.8 m/s². v = 5+4.8×3 ≈ 19.4 m/s → risposta più vicina: 16.9 m/s." },
  { q: "Per calcolare la velocità senza conoscere il tempo, si usa:", opts: ["v = v₀+at","s = v₀t+½at²","v² = v₀²+2as","F = ma"], correct: 2, exp: "v² = v₀² + 2as lega velocità, accelerazione e spazio senza il tempo." },
] as const
    ...
    <>
      <div className="card">
        ...
        <div style={{display:'grid',gap:'0.5rem',marginTop:'0.5rem'}}>
          {[['1','Disegna il sistema','Schizza l\'oggetto e identifica tutte le forze.'],['2','Schema delle forze (FBD)','Rappresenta ogni forza come freccia.'],['3','Scegli gli assi','Per il piano inclinato usa assi parallelo/perpendicolare.'],['4','Equazioni di Newton','Applica ΣF = ma su ogni asse.'],['5','Calcola e verifica','Risolvi per le incognite e verifica le unità.']
          ].map(([n,title,desc]) => (
            <div key={n} style={{display:'flex',gap:'0.75rem',alignItems:'flex-start',padding:'0.6rem 0.8rem',background:'var(--soft)',borderRadius:8,border:'1px solid var(--border)'}}>
              ...
              <div><div style={{color:'var(--primary)',fontWeight:600,fontSize:'0.9rem'}}>{title}</div><div style={{color:'var(--muted)',fontSize:'0.83rem',marginTop:2}}>{desc}</div></div>
            </div>
          ))}
        </div>
        ...
        <div className="info-box example">
          <div><strong>Esempio:</strong> blocco 8 kg, piano 25°, v₀=6 m/s, μ=0.3.<br />
          N=71.0 N, Fₐ=21.3 N, a=−6.81 m/s², t_stop=0.88 s, s_max=2.64 m.</div>
        </div>
      </div>
      <div className="sim-card">
        ...
        {stopped && <div className="info-box warn" style={{marginBottom:'0.5rem'}}>Oggetto fermo — posizione congelata.</div>}
        {mode==='orizzontale' ? <SceneOriz m={m} F={F} mu={mu} t={t} a={a} v0={v0} /> : <SceneIncl m={m} mu={mu} theta={theta} t={t} a={a} v0={v0} />}
